const { PROJECT_ID, ENDPOINT, API_KEY, DATABASE_ID, STORAGE_BUCKET_ID, headers, api, delay } = require("./config");
...
async function deleteAllDocuments(collectionId) {
  ...
  let hasMore = true;
  while (hasMore) {
    const res = await api(`/databases/${DATABASE_ID}/collections/${collectionId}/documents?limit=100&offset=0`, "GET");
    if (!res || !res.documents || res.documents.length === 0) { hasMore = false; break; }
    for (const doc of res.documents) {
      await api(`/databases/${DATABASE_ID}/collections/${collectionId}/documents/${doc.$id}`, "DELETE");
      total++;
      await delay(100);
    }
...
async function deleteAllFiles() {
  ...
  let hasMore = true;
  while (hasMore) {
    const res = await api(`/storage/buckets/${STORAGE_BUCKET_ID}/files?limit=100`, "GET");
    if (!res || !res.files || res.files.length === 0) { hasMore = false; break; }
    for (const file of res.files) {
      await api(`/storage/buckets/${STORAGE_BUCKET_ID}/files/${file.$id}`, "DELETE");
      total++;
      await delay(100);
    }
...
async function reseed() {
  ...
  const collections = ["recommendations", "visit_photos", "visits", "items", "customers"];
  ...
  console.log("\nClearing storage bucket files...");
  const fileCount = await deleteAllFiles();
  console.log("  Deleted " + fileCount + " files from storage");
  ...
  for (const c of customers) {
    ...
    const doc = await api("/databases/" + DATABASE_ID + "/collections/customers/documents", "POST", {
      ...
      data: {
        ...
        latitude: c.latitude, longitude: c.longitude, cropType: c.cropType,
        contactName: c.contactName || null, contactPhone: c.contactPhone || null,
      },
  ...
  console.log("\n=== Creating Items (with random expiry dates in next 3 months) ===");
  for (const item of items) {
    const expiryDate = randomExpiryDate();
    console.log("  Creating: " + item.name + " -> expires " + expiryDate);
    const doc = await api("/databases/" + DATABASE_ID + "/collections/items/documents", "POST", {
      documentId: "unique()",
      data: {
        name: item.name, category: item.category, unit: item.unit,
        tallyCode: item.tallyCode, expiryDate: expiryDate,
      },
      read: ["role:all"], write: ["role:all"],
    });
  ...
  for (let i = 0; i < visitsSeed.length; i++) {
    ...
    const daysUntil = [7, 14, 3, 7, 10, 14, 5][i] || 7;
  ...
  for (let i = 0; i < recommendationsSeed.length; i++) {
    ...
    await api("/databases/" + DATABASE_ID + "/collections/recommendations/documents", "POST", {
      documentId: "unique()",
      data: {
        visitId: visitIds[i], customItem: r.customItem,
        dosage: r.dosage, quantity: r.quantity, notes: r.notes,
      },
      read: ["role:all"], write: ["role:all"],
    });
    await delay(150);
